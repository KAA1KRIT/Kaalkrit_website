...
export function ScrollExpandHero() {
  ...
  useEffect(() => {
    ...
    const setInteractiveState = (
      ...
      compactContent.toggleAttribute(
        "inert",
        !isCompact && compactOpacity < 0.05,
      );
      expandedActions.toggleAttribute(
        "inert",
        !isCompact && expandedOpacity < 0.7,
      );
    };
    ...
    const update = () => {
      ...
      const scale = Math.min(
        maxScale,
        Math.max(1, 1 + (targetScale - 1) * eased),
      );
      ...
      const summaryOpacity =
        summaryProgress * summaryProgress * (3 - 2 * summaryProgress);

      section.style.setProperty("--hero-progress", eased.toFixed(4));
      section.style.setProperty(
        "--hero-compact-opacity",
        compactOpacity.toFixed(4),
      );
      section.style.setProperty(
        "--hero-expanded-opacity",
        expandedOpacity.toFixed(4),
      );
      section.style.setProperty(
        "--hero-summary-opacity",
        summaryOpacity.toFixed(4),
      );
      section.style.setProperty(
        "--hero-expanded-y",
        `${Math.round((1 - expandedOpacity) * 28)}px`,
      );
        ...
        <div className="scroll-expand-hero__content public-container">
          ...
          <div className="scroll-expand-hero__expanded-copy">
            ...
            <div
              ...
              className="scroll-expand-hero__expanded-actions"
            >
              <TrackedLink
                href="#projects"
                event="primary_cta_click"
