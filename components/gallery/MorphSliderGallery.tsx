...
const MorphSliderRegistry = dynamic(
  () =>
    import("./MorphSliderRegistry").then(
      (module) => module.MorphSliderRegistry,
    ),
  {
    ssr: false,
    loading: () => <GallerySkeleton label="Loading project media" />,
  },
);
...
export function MorphSliderGallery({ items }: { items: GalleryItem[] }) {
  ...
  if (readyItems.length < 2) {
    return (
      <GalleryEmptyState description="Project documentation is shared when it is ready for publication." />
    );
  }
  return (
    <GalleryErrorBoundary label="Project media">
      <MorphSliderRegistry items={readyItems} />
