...
import { collaborationModes } from '@/content/partner';
        ...
        <div className="partnership__body">
          <ul className="collaboration-list">
            {collaborationModes.slice(0, 4).map((mode, index) => (
              <li key={mode.id}><span>0{index + 1}</span><div><h3>{mode.title}</h3><p>{mode.description}</p></div></li>
            ))}
          </ul>
          <div className="partnership__route">
            ...
            <Button href={mailto('Partnership with Team KAALKRIT')} variant="primary" trailing={<span aria-hidden="true">↗</span>}>Email the team</Button>
          </div>
        </div>
      </div>
    </section>
